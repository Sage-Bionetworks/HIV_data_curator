...
schema_to_display_lookup <- data.frame(schema_name, display_name)
  ...
  observeEvent(
    ...
    if (existing_manifestID == '') {
      ...
      manifest_url <- metadata_model$getModelManifest(paste0("HIV ", input$template_type), template_type, filenames = as.list(filename_list))
      ...
      manifest_url <- metadata_model$populateModelManifest(paste0("HIV ", input$template_type), manifest_entity$path, template_type)
      toggle('text_div3')
  ...
  observeEvent(
    ...
    if (length(annotation_status) != 0) {
      ...
      filled_manifest <- metadata_model$populateModelManifest(paste0("HIV ", input$template_type), input$file1$datapath, template_type)
